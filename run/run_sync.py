...
class Run(QThread):
    ...
    def __run_device(device: Device, script: Script):
        ...
        while step < total_steps:
            ...
            # kill进程
                ...
                if point:
                    logger_run.info(f"目标图上模板图坐标匹配结果为: {point}, 当前匹配次数为第1次---{script.script_content[step]}")
                else:
    ...
    def __task() -> List[Task]:
        today = get_date()  # 获得当天日期
        tasks = TaskService().select_by_date(today)
        if not tasks:
            logger_run.warning(f"{today} 没有发布新任务")

        # 当日无最新发布任务，查询历史任务并执行，最多查询前7天任务
        date_ = 1
        while not tasks and date_ < 27:
            yesterday = get_date(before_today=True, n=date_)
            tasks = TaskService().select_by_date(yesterday)
            if not tasks:
                logger_run.info(f"{yesterday} 没有发布新任务")
                time.sleep(1)
                date_ += 1
        if not tasks:
            logger_run.info("近7天未发布新任务, 请添加新任务")
            return tasks
        return tasks
