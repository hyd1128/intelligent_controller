...
from q_threads.module_varchar import download_script
...
class DownloadAppThread(QThread):
    ...
    def run(self):
        ...
        download_app_script = download_script
        for s1 in range(len(download_app_script)):
            for d1 in range(len(suitable_device_ids)):
                ...
                # 7、点击进入应用界面
                # 8、匹配下载图标并点击下载应用
                run_download_app(download_app_script[s1], suitable_device_ids[d1])
