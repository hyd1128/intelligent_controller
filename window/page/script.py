...
from window.dialog.save_script import SaveScriptDialog
...
class ScriptPage:
    ...
    def table_widget(self):
        ...
        # 设置表头
        self.table.setHorizontalHeaderLabels(["应用包名", "脚本名称", "脚本内容", "操作"])
    ...
    def set_table_data(self):
        # scripts = ScriptStore().select()
        scripts = ScriptService().select()
        ...
        for row, script in enumerate(scripts):
            self.table.setItem(row, 0, QtWidgets.QTableWidgetItem(script.app))
            self.table.setItem(row, 1, QtWidgets.QTableWidgetItem(script.script_name))
            self.table.setItem(row, 2, QtWidgets.QTableWidgetItem(script.script_content))
            delete_btn = QtWidgets.QPushButton()
            delete_btn.setText("删除")
            delete_btn.setStyleSheet("background:red;color:white;")
            delete_btn.clicked.connect(lambda checked, current=script: self.delete(current.script_name))
    ...
    def save_dialog(self, app=None):
        ...
        SaveScriptDialog(app, self).exec()

    # 删除应用
    def delete(self, name):
        # ScriptStore().delete(name)
        ScriptService().delete_by_name(name)
        self.set_table_data()
